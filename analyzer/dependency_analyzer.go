...
var (
	...
	// Problematic licenses - kept for future implementation
	...
	// License patterns in files - kept for future implementation
...
func (a *DependencyAnalyzer) Analyze(node interface{}, fset *token.FileSet) []*Issue {
	var issues []*Issue

	// Get filename if possible
	filename := ""
	if astNode, ok := node.(ast.Node); ok && astNode.Pos().IsValid() && fset != nil {
		filename = fset.Position(astNode.Pos()).Filename
	}

	// Handle nil node for project-level analysis
	if node == nil {
		// For project-level analysis, analyze go.mod if it exists
		if filename == "go.mod" {
			return a.analyzeGoMod()
		}
		return issues
	}
	...
	// Check the go.mod file once per project
...
func (a *DependencyAnalyzer) checkImport(importPath string, pos token.Position) []*Issue {
	...
	if msg, deprecated := deprecatedPackages[importPath]; deprecated {
		issues = append(
			issues, &Issue{
				Type:     IssueDependencyVulnerable,
...
func (a *DependencyAnalyzer) checkGoMod() []*Issue {
	...
	for scanner.Scan() {
		...
		if inRequire && line != "" && !strings.HasPrefix(line, "//") {
			lineIssues := a.processRequireLine(line, goModPath, lineNum, &messageBuilder)
			issues = append(issues, lineIssues...)
		}

		if inReplace && line != "" && !strings.HasPrefix(line, "//") {
			replaceIssue := a.processReplaceLine(line, goModPath, lineNum)
			if replaceIssue != nil {
...
func (a *DependencyAnalyzer) processRequireLine(line, goModPath string, lineNum int, messageBuilder *strings.Builder) []*Issue {
	var issues []*Issue
	...
	if indirectIssue := a.checkIndirectDependency(line, pkg, goModPath, lineNum); indirectIssue != nil {
		issues = append(issues, indirectIssue)
	}
...
func (a *DependencyAnalyzer) checkIndirectDependency(line, pkg, goModPath string, lineNum int) *Issue {
...
func (a *DependencyAnalyzer) processReplaceLine(line, goModPath string, lineNum int) *Issue {
...
func (a *DependencyAnalyzer) checkGoSum() []*Issue {
	...
	if issue := a.checkGoSumExists(goSumPath); issue != nil {
		return []*Issue{issue}
	}

	// Check if go.sum is empty
	if issue := a.checkGoSumEmpty(goSumPath); issue != nil {
		issues = append(issues, issue)
	}

	// Check for version conflicts
	issues = append(issues, a.checkVersionConflicts(goSumPath)...)

	return issues
}

func (a *DependencyAnalyzer) checkGoSumExists(goSumPath string) *Issue {
...
func (a *DependencyAnalyzer) checkGoSumEmpty(goSumPath string) *Issue {
...
func (a *DependencyAnalyzer) checkVersionConflicts(goSumPath string) []*Issue {
	var issues []*Issue
...
func (a *DependencyAnalyzer) checkPackageVulnerabilities(
	pkg, version, goModPath string, lineNum int, messageBuilder *strings.Builder,
) []*Issue {
	vulnIssues := make([]*Issue, 0, 2)

	// Check for vulnerabilities using VulnerabilityChecker
	vulns, err := a.vulnChecker.CheckPackage(pkg, version)
	...
	for _, vuln := range vulns {
		...
		case osvSeverityCritical, osvSeverityHigh:
			severity = SeverityLevelHigh
		case osvSeverityLow:
			severity = SeverityLevelLow
		}
		...
		_, _ = fmt.Fprintf(messageBuilder, "Vulnerable dependency: %s %s", pkg, version)
		if vuln.CVE != "" {
	...
	// If no vulnerabilities found via checker, try fallback to static vulnerability data
	if len(vulnIssues) == 0 {
...
func (a *DependencyAnalyzer) isVulnerableVersion(version, vulnSpec string) bool {
	// TODO: fix simple version comparison - in production would use a proper semver library
...
func (a *DependencyAnalyzer) isOldVersion(version string) bool {
	// Check if a version is older than 2 years
...
func (a *DependencyAnalyzer) analyzeGoMod() []*Issue {
	var issues []*Issue

	// Try to read the go.mod file
	goModPath := filepath.Join(a.projectPath, "go.mod")
	if _, err := os.Stat(goModPath); os.IsNotExist(err) {
		return nil
	}

	// TODO: Parse go.mod and analyze dependencies
